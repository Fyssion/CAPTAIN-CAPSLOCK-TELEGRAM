...
async def on_message(event):
    ...
    peer_id, user_id = event.chat_id, telethon.utils.get_peer_id(message.from_id)
...
async def togglegroup_command(event):
    ...
    if new_state:
        await event.respond(
            "SHOUTING AUTO RESPONSE IS NOW **OPT-OUT** FOR THIS CHAT",
            parse_mode="markdown",
        )
    else:
        await event.respond(
            "SHOUTING AUTO RESPONSE IS NOW **OPT-IN** FOR THIS CHAT",
            parse_mode="markdown",
        )
...
async def toggle_command(event):
    message = event.message
    chat_id = (
        event.chat_id if not isinstance(message.to_id, tl.types.PeerUser) else None
    )
    from_id = telethon.utils.get_peer_id(event.message.from_id)
    new_state = await event.client.db.toggle_user_state(from_id, chat_id)
    if new_state:
...
async def remove_command(event):
    ...
    if event.is_group and event.is_channel:
        participant = await event.client(
            tl.functions.channels.GetParticipantRequest(
                channel=message.to_id, user_id=message.from_id
            )
        )
        if not (
            isinstance(participant, tl.types.ChannelParticipantAdmin)
            and participant.admin_rights.delete_messages
        ):
            await event.respond(
                "YOU MUST BE AN ADMIN WITH DELETE MESSAGES PERMISSION TO RUN THIS COMMAND."
            )
            return
    ...
    to_delete = []
    if await event.client.db.delete_shout(
        message.to_id.chat_id, message.reply_to_msg_id
    ):
